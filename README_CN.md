...
**QQ 群聊机器人内核**，对接 [NapCat](https://github.com/NapNeko/NapCatQQ) / [OneBot 11](https://github.com/botuniverse/onebot-11)，用 [LangGraph](https://github.com/langchain-ai/langgraph) Agent 跑在 [FastAPI](https://fastapi.tiangolo.com/) 上。群聊按说话人分片，避免把 A 没说完的话题答给 B。

热上下文进 **Redis**，温数据落 **Postgres**，冷检索走 **pgvector**（本地哈希向量，不调外部 embedding API）。

> 不是又一个「整群共用一条会话」的 bot。Luopita 把触发、路由、组装、压缩和存储拆开。

当前版本 **0.2.0**。完整说明见 **[文档站](https://rovinax.github.io/luopita/)**。默认 README 为英文：[README.md](README.md)。
...
- **主人斜杠命令**与自然语言定时任务（`/cron` 不经模型）
- **开放承诺：** 从对话抽出「明天提醒我交周报」，到期限频跟进
- **说话样例**按场景注入 system，不是会话记忆；**群友画像**只描述当前说话人
...
QQ 机器人 · NapCat · OneBot 11 · 群聊机器人 · LangGraph Agent · FastAPI chatbot · 群聊上下文 · 按说话人分片 · Redis · Postgres · pgvector · Python 3.12
